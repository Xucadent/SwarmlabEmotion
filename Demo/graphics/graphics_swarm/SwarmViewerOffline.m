        ...
        function self = update(self, swarm, time, pos_current, map, step)
            ...
            if (time-self.time_of_last_frame) >= self.output_rate
                ...
                if self.plot_initialized == 0 % Init plot
                    ...
                    size_agents = repmat(20, swarm.nb_agents, 1); % size_agents is size of dot drawn in scatter3.
                    
                    pos_current_matrix = reshape(pos_current,2,[]);
                    pn = pos_current_matrix(1,:);
                    pe = pos_current_matrix(2,:);
            %%        pd = pos_current_matrix(2,:);

                    self.scatter_handle = scatter(pe, pn, size_agents, colours', 'filled');
                    
                    % Draw wakes
                    for agent = 1:swarm.nb_agents
                        pn = self.wakes(:,2*(agent-1)+1);
                        pe = self.wakes(:,2*(agent-1)+2);
                 %%       pd = self.wakes(:,2*(agent-1)+2);
                        hold on;
                        self.lines_handle(agent) = plot(...
                            pe, pn, 'Color', colours(:,agent)');
                    end
           ...
           %%         zlabel('Z position [m]');
                    ...
                    size_agents = repmat(20, swarm.nb_agents, 1);
                    colours = swarm.get_colors();
         ...
         %%               pd = self.wakes(:,3*(agent-1)+3);
                        hold on;
                        set(self.lines_handle(agent), 'Xdata', pe, 'Ydata', pn) %, 'CData', colours(:,agent)');
